...
export const classroomService = {
  ...
  async getClassroomsByTeacher(teacherId: string): Promise<Classroom[]> {
    const { data, error } = await supabase.from('classrooms').select('*').eq('teacher_id', teacherId).order('name');
    if (error) { console.error(error.message); return []; }
    return data as Classroom[];
  },
  ...
  async createClassroom(name: string, schoolId: string, teacherId: string): Promise<Classroom | null> {
  ...
  async updateClassroom(
    id: string,
    updates: { name?: string; school_id?: string; teacher_id?: string }
